...
Sil_Omar_normline_list_test = [['Ha', 6562.819, [[-1300, -1000], [1700, 1850]], r'H$\alpha$ 6563'], ['Hb', 4861.333, [[1387, 1927], [-1000, -700]], r'H$\beta$ 4861'], ['Hy', 4340.472, [[-1000, -700], [1000, 1500]], r'H$\gamma$ 4340']]
...
for i,line in enumerate(Sil_Omar_normline_list_test):
    # line_oud = apo_linelist[i]
    k = 2
    print('asdf', line[1])
    start = line[2][0][0]-200
    stop = line[2][1][1]+200
    wl = merc_file.wl_rebin
    flux = merc_file.flux_rebin

    lw,nlf,nnf,lf,fit = airmass.normalize(wl,flux,line[2][0][0],line[2][0][1],line[2][1][0],line[2][1][1],line[2][0][0]-200,line[2][1][1]+200)
