...
# asdf = np.array([all_ews_n,all_phases])
print np.ndarray.max(all_ews_n)
for i,ew in enumerate(all_ews_n):
    print i, all_phases[i],ew
...
for file in filelist_apo:
    ...
    ews = []
    errs = []
    phase = airmass.timeanddate(file)[0]
...
# np.save('C:\peter\School\Master Scriptie\Data\EW\eshelhb-5.npy',dat)
# plt.scatter(all_phases,all_ews_n)
# plt.show()
# # ------------Fitting-----------------------------------------------------------
#
...
# # pars, corr = curve_fit(func, phase1, ew1, p0=[1, 0], sigma=eb1,absolute_sigma=True)
# # # print 'fit corr = ', corr
# # plt.errorbar(ew1, ew2, xerr=eb2,yerr=eb2, fmt='o')
...
# # plt.title(r'EW comparision of He I 5876 and H$\beta $ (eShel)')
# # plt.xlim([0.85,1.1])
# # plt.xlim([0.85,1.1])
# # plt.xlabel(r'EW H$\beta$ ($\AA$)')
# # plt.ylabel(r'EW He I 5875 ($\AA$)')
