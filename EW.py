...
savefolder = r'C:\peter\School\Master Scriptie\Data\EW\test'
...
#
# np.save(savefolder + r'\lapalmahe5875.npy',dat)
...
def my_sin3(x,  amplitude, phase, offset):
...
for item in dat1:
    print item
lineused = dat1[3][0]
print lineused[0],str(int(lineused[1]))
linename = lineused[0] +' '+ str(int(lineused[1]))
print linename
...
fit2 = curve_fit(my_sin3, phases2, ew2,  p0=p1, sigma=eb2, absolute_sigma=True)
# # print fit[0][0]/(2*np.pi)
fit1 = curve_fit(my_sin3, phases1, ew1, p0=p1,  sigma=eb1, absolute_sigma=True)
# print fit1[0]
# print fit2[0]
#
...
lp_phase_er1 = 0.0625631105713
lp_phase_er2 = 0.135780272674
es_phase_er1 = 0.0660945013665
es_phase_er2 = 0.139311663469
# # ---------------plotting----------
t= np.linspace(0,1,50)
# data_fit = my_sin2(t, *fit1[0])
# data_fit2 = my_sin2(t, *fit2[0])
# maxi = float(minimize_scalar(lambda x: -my_sin2(x, *fit1[0]), bounds=[0,1], method='bounded').x)
# maxi2 = float(minimize_scalar(lambda x: -my_sin2(x, *fit2[0]), bounds=[0,1], method='bounded').x)
data_fit = my_sin3(t, *fit1[0])
data_fit2 = my_sin3(t, *fit2[0])
maxi = float(minimize_scalar(lambda x: -my_sin3(x, *fit1[0]), bounds=[0,1], method='bounded').x)
maxi2 = float(minimize_scalar(lambda x: -my_sin3(x, *fit2[0]), bounds=[0,1], method='bounded').x)
#
f, (ax1, ax2) = plt.subplots(2, sharex=True)
# plt.suptitle(r'H$\beta$',size=18 )
plt.suptitle(linename.replace("_"," "),size=18 )
ax1.set_title('HERMES',size=12)
ax1.axvline(maxi,linestyle = 'dashed',c='k')
ax1.axvspan(maxi-lp_phase_er1, maxi+lp_phase_er1, color='k', alpha=0.2, lw=0)
ax1.axvspan(maxi-lp_phase_er2, maxi+lp_phase_er2, color='k', alpha=0.2, lw=0)
if maxi+lp_phase_er1 > 1:
    ax1.axvspan(0, maxi+lp_phase_er1-1, color='k', alpha=0.2, lw=0)
if maxi+lp_phase_er2>1:
    ax1.axvspan(0, maxi+lp_phase_er2-1, color='k', alpha=0.2, lw=0)
if maxi-lp_phase_er1 <0:
    ax1.axvspan(maxi-lp_phase_er1+1, 1, color='k', alpha=0.2, lw=0)
if maxi-lp_phase_er2 <0 :
    ax1.axvspan(maxi-lp_phase_er2+1, 1, color='k', alpha=0.2, lw=0)
# ax1.fill_between(t, 0, 1, where=y < -theta, facecolor='red', alpha=0.5, transform=trans)
ax2.set_title('eShel',size=12)
ax2.axvline(maxi2,linestyle = 'dashed',c='k')
ax2.axvspan(maxi2-es_phase_er1, maxi2+es_phase_er1, color='k', alpha=0.2, lw=0)
ax2.axvspan(maxi2-es_phase_er2, maxi2+es_phase_er2, color='k', alpha=0.2, lw=0)
if maxi2+es_phase_er1 > 1:
    ax2.axvspan(0, maxi2+es_phase_er1-1, color='k', alpha=0.2, lw=0)
if maxi2+es_phase_er2>1:
    ax2.axvspan(0, maxi2+es_phase_er2-1, color='k', alpha=0.2, lw=0)
if maxi2-es_phase_er1 <0:
    ax2.axvspan(maxi2-es_phase_er1+1, 1, color='k', alpha=0.2, lw=0)
if maxi2-es_phase_er2 <0 :
    ax2.axvspan(maxi2-es_phase_er2+1, 1, color='k', alpha=0.2, lw=0)
...
plt.savefig(r'C:\peter\School\Master Scriptie\figures\EWs\together/' +linename+'2n.pdf')
plt.show()
# ---------------EW-EW-Comparison-------------------------------------------------
